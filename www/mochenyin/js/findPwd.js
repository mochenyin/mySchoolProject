/**
 * Created by SWSD on 2017-02-24.
 */
function findPwdController($scope){
    let type=false;
    let str;
    ...
    $scope.setTime=function(){
        let btn=document.getElementById('findPwdBtn');
    ...
    $scope.clickToBlock=function(){
        if($scope.checkTelNumber()==true&&$scope.catchMsg()==true){
            $('#updataPwd').css('display','block');
        }
    };
    $scope.findPwd=function(){
        $('#updataPwd').css('display','none');
    ...
    $scope.clickToRegister=function(){
        if( $scope.getPwd()==true&&$scope.checkPwdAgain()==true){
            let dat={
                email:$('#number').val(),
                pwd:hex_md5($('#pwd').val())
            };
            $.post('/api/updatePwd',dat,function(res){
                if(res.text=='ok'){
                        $scope.text = '修改成功';
                    boxshow($scope.text);
                    setTimeout(function(){
